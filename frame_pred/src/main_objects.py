...
FFP_TRAIN_DATASET_PATH = '../../../../../vast/cg4177/raw_data1/Dataset_Student/unlabeled/test'
FFP_TEST_DATASET_PATH = '../../../../../vast/cg4177/raw_data1/Dataset_Student/val'
...
def train(cfg):
    ...
    train_dataset = MovingObjectsDataset(
        root_dir=f'{FFP_TRAIN_DATASET_PATH}',
    ...
    test_dataset = MovingObjectsDataset(
        root_dir=f'{FFP_TEST_DATASET_PATH}',
        transform=transforms.Compose(
                [
                    transforms.Resize((img_size[0], img_size[1])),
                    transforms.ToTensor(),
        ...
        for step, [past_frames, true_future_frames] in enumerate(train_loader):
            ...
            if main_worker:
                if (step + 1) % print_freq == 0:
                    ...
                    plt.imsave(
                        f"../results/{file_identifier}/{current_step:06d}/train_feed_seq.png", past_frames.cpu().permute(1, 2, 0).numpy()
                    )
                    plt.imsave(
